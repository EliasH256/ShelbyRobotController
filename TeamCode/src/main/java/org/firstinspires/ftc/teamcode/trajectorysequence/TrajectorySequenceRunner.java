...
public class TrajectorySequenceRunner {
    private final static String TAG = "SJH_TSR";

    private static boolean tsrDebugging = false;
    ...
    DriveSignal update(Pose2d poseEstimate, Pose2d poseVelocity) {
        ...
        if(packet == null)
        {
            ...
            CommonUtil.getInstance().setTelemetryPacket(packet);
        }
        ...
        if (currentTrajectorySequence != null) {
            ...
            if (currentSegment instanceof TrajectorySegment) {
                ...
                if (isNewTransition)
                {
                    RobotLog.dd(TAG, "Starting segment %d at %.2f from %s to %s",
                        currentSegmentIndex, now, currentSegment.getStartPose().toString(),
                        currentSegment.getEndPose().toString());
                    follower.followTrajectory(currentTrajectory);
                }

                if (!follower.isFollowing()) {
                    RobotLog.dd(TAG, "Done following segment %d at %.2f in %.2f",
                        currentSegmentIndex, now, deltaTime);
                    currentSegmentIndex++;
                    ...
                    if (tsrDebugging) {
                        RobotLog.dd(TAG, "  at %s error %s",
                                poseEstimate, lastPoseError);
                    }
                }
        ...
        CommonUtil.getInstance().getFtcDashboard().sendTelemetryPacket(packet);
    ...
    )
    {
        ...
        {
            TelemetryPacket tp = CommonUtil.getInstance().getTelemetryPacket();
            if(tp == null) tp = new TelemetryPacket();
            fieldOverlay = tp.fieldOverlay();
        }
    ...
    public void cancelFollowing() {
        RobotLog.ii(TAG, "Trajectory sequencer being cancelled");
        currentTrajectorySequence = null;
    }

    public void draw(Pose2d poseEstimate)
    {
        draw(null, null, null, null, poseEstimate);
    }

    public void drawRoute()
    {
        draw(null, currentTrajectorySequence, null, null, null);
    }
}
